...
async function main(): Promise<void> {
    ...
    if (args.includes('--target') && !target) {
        console.error('Usage: bun run scripts/build-executable.ts [--target <bun-platform[-arch[-variant]]>] [--outdir dist-exe] [--name agentchat] [--with-web-assets]');
        console.error('   or: bun run scripts/build-executable.ts --all [--outdir dist-exe] [--name agentchat] [--with-web-assets]');
        process.exit(1);
    }
