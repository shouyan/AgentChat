...
export async function handleAuthCommand(args: string[]): Promise<void> {
    ...
    if (subcommand === 'status') {
        ...
        console.log(chalk.gray(`  AGENTCHAT_API_URL: ${configuration.apiUrl}`))
        ...
        if (!hasToken) {
            ...
            console.log(chalk.gray('    2. Read ~/.agentchat/settings.json on the server'))
            console.log(chalk.gray('    3. Ask your server administrator (if token is set via env var)'))
            console.log('')
            console.log(chalk.gray('  Then run: agentchat auth login'))
        }
        return
    }
...
function showHelp(): void {
    console.log(`
${chalk.bold('agentchat auth')} - Authentication management

${chalk.bold('Usage:')}
  agentchat auth status            Show current configuration
  agentchat auth login             Enter and save CLI_API_TOKEN
  agentchat auth logout            Clear saved credentials

${chalk.bold('Token priority (highest to lowest):')}
  1. CLI_API_TOKEN environment variable
  2. ~/.agentchat/settings.json
