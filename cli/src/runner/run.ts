...
export async function startRunner(): Promise<void> {
  ...
  let requestShutdown: (source: ShutdownSource, errorMessage?: string, options?: { restart?: boolean }) => void;
    ...
    const cleanupAndShutdown = async (source: ShutdownSource, errorMessage?: string) => {
      ...
      if (restartAfterShutdown) {
        ...
        try {
          ...
          logger.debug('[RUNNER RUN] Failed to restart runner after shutdown', restartError);
        }
      }
