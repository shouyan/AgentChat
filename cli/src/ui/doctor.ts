 ...
 * for AgentChat CLI including configuration, runner status, logs, and links
 */
...
export async function runDoctorCommand(filter?: 'all' | 'runner'): Promise<void> {
    ...
    if (filter === 'all') {
        ...
        if (hasToken) {
            ...
            console.log(chalk.gray('  Run `agentchat auth login` to configure or set CLI_API_TOKEN env var'));
        }
    ...
    try {
        ...
        if (allProcesses.length > 0) {
            ...
            console.log(chalk.red('❌ No AgentChat processes found'));
        }

        if (filter === 'all' && allProcesses.length > 1) { // More than just current process
            console.log(chalk.bold('\n💡 Process Management'));
            console.log(chalk.gray('To clean up runaway processes: agentchat doctor clean'));
        }
    } catch (error) {
