...
function readWorktreeFromEnv(): WorktreeInfo | null {
    const basePath = process.env.AGENTCHAT_WORKTREE_BASE_PATH?.trim();
    const branch = process.env.AGENTCHAT_WORKTREE_BRANCH?.trim();
    const name = process.env.AGENTCHAT_WORKTREE_NAME?.trim();
    const worktreePath = process.env.AGENTCHAT_WORKTREE_PATH?.trim();
    const createdAtRaw = process.env.AGENTCHAT_WORKTREE_CREATED_AT?.trim();
