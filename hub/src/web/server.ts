...
function createWebApp(options: {
    ...
    app.get('/health', (c) => c.json({
        ...
        feishu: options.getFeishuStatus?.() ?? null,
    }))
    ...
    app.route('/api', createRoomsRoutes(options.getSyncEngine))
    app.route('/api', createMessagesRoutes(options.getSyncEngine))
    app.route('/api', createPermissionsRoutes(options.getSyncEngine))
    app.route('/api', createMachinesRoutes(options.getSyncEngine))
    app.route('/api', createGitRoutes(options.getSyncEngine))
    app.route('/api', createPushRoutes(options.store, options.vapidPublicKey))
    app.route('/api', createTemplateRoutes(options.store, options.getSseManager))
    app.route('/api', createVoiceRoutes())

    // Skip static serving in relay mode, show helpful message on root
    if (options.relayMode) {
        const officialUrl = options.officialWebUrl || 'https://app.agentchat.run'
        app.get('/', (c) => {
            return c.html(`<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>AgentChat Hub</title></head>
...
export async function startWebServer(options: {
    ...
    console.log(`[Web] AgentChat hub listening on ${configuration.listenHost}:${configuration.listenPort}`)
