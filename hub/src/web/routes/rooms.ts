...
export function createRoomsRoutes(getSyncEngine: () => SyncEngine | null): Hono<WebAppEnv> {
  ...
  app.patch('/rooms/:id', async (c) => {
    ...
    return c.json(RoomResponseSchema.parse({
  ...
  app.patch('/rooms/:id/tasks/:taskId', async (c) => {
    ...
    return c.json(RoomResponseSchema.parse({ room: engine.updateRoomTask(roomId, c.req.param('taskId'), namespace, parsed.data) }))
  })
