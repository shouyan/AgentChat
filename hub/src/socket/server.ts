...
export function isSocketOriginAllowed(origin: string | null, requestHost: string | null, corsOrigins: string[], allowAllOrigins: boolean): boolean {
    if (!origin || allowAllOrigins || corsOrigins.includes(origin)) {
        return true
    }

    const originHost = normalizeHost(origin)
    const requestHostname = normalizeHost(requestHost)
...
export function createSocketServer(deps: SocketServerDeps): {
    ...
    const engine = new Engine({
        ...
        allowRequest: async (req) => {
            ...
            if (isSocketOriginAllowed(origin, requestHost, corsOrigins, allowAllOrigins)) {
                return
            }
    ...
    const idleTimeoutMs = resolveEnvNumber('AGENTCHAT_TERMINAL_IDLE_TIMEOUT_MS', DEFAULT_IDLE_TIMEOUT_MS)
    const maxTerminals = resolveEnvNumber('AGENTCHAT_TERMINAL_MAX_TERMINALS', DEFAULT_MAX_TERMINALS)
