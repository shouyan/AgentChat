...
export function NewSession(props: {
    ...
    const resolvedMachineId = machineId ?? props.machines[0]?.id ?? null

    const selectedMachine = useMemo(
        () => (resolvedMachineId ? props.machines.find((machine) => machine.id === resolvedMachineId) ?? null : null),
        [resolvedMachineId, props.machines]
    )
    ...
    const recentPaths = useMemo(
        () => getRecentPaths(resolvedMachineId),
        [getRecentPaths, resolvedMachineId]
    )

    const allPaths = useDirectorySuggestions(resolvedMachineId, sessions, recentPaths)
    ...
    useEffect(() => {
        ...
        if (!resolvedMachineId || pathsToCheck.length === 0) {
            setPathExistence({})
            return () => { cancelled = true }
        }

        void props.api.checkMachinePathsExists(resolvedMachineId, pathsToCheck)
            .then((result) => {
                if (cancelled) return
    ...
    }, [resolvedMachineId, pathsToCheck, props.api])
    ...
    async function handleCreate() {
        if (!resolvedMachineId || !directory.trim()) return
        ...
        try {
            ...
            if (result.type === 'success') {
                ...
                setLastUsedMachineId(resolvedMachineId)
                addRecentPath(resolvedMachineId, directory.trim())
    ...
    const canCreate = Boolean(resolvedMachineId && directory.trim() && !isFormDisabled)
    ...
    return (
        <div className="flex flex-col divide-y divide-[var(--app-divider)]">
            ...
            <DirectorySection
                ...
                isDisabled={isFormDisabled}
                isBrowseDisabled={!resolvedMachineId}
            ...
            <DirectoryBrowserDialog
                ...
                initialPath={directory || recentPaths[0] || ''}
