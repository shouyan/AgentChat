...
export function InboxSidebar(props: {
  ...
  const { rooms, isLoading: roomsLoading, error: roomsError, refetch: refetchRooms } = useRooms(props.api)
  const { sessions, isLoading: sessionsLoading, error: sessionsError, refetch: refetchSessions } = useSessions(props.api)
    ...
    for (const room of rooms) {
      for (const role of room.state.roles) {
        ...
        if (role.assignmentMode === 'spawn_new') {
          roomSpawnedSessionIds.add(role.assignedSessionId)
        }
      }
    }

    return sessions.filter((session) => {
      if (session.metadata?.roomSpawned) {
        return false
      }
      if (session.metadata?.roomId && knownRoomIds.has(session.metadata.roomId)) {
        return false
      }
      if (roomSpawnedSessionIds.has(session.id)) {
        return false
      }
      return true
    })
