...
export function MachineStatusCard(props: {
    ...
    runnerEnvContent?: string
    onRunnerEnvChange?: (content: string) => void
    onReloadRunnerEnv?: () => void
    onSaveRunnerEnv?: () => void
            ...
            <CardContent className="space-y-4">
                ...
                <div className="grid gap-2 text-sm text-[var(--app-hint)] sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <div className="mt-1 break-all text-[var(--app-fg)]">{props.machine.metadata?.agentchatHomeDir ?? '—'}</div>
                    </div>
                </div>
                ...
                <div className="space-y-2">
                    <div className="flex items-center justify-between gap-3">
                        <div className="text-xs uppercase tracking-wide text-[var(--app-hint)]">Runner environment</div>
                        {props.runnerEnvPath ? <div className="text-xs text-[var(--app-hint)]">{props.runnerEnvPath}</div> : null}
                    </div>
                    <div className="rounded-lg border border-[var(--app-border)] bg-[var(--app-secondary-bg)] p-3">
                        <textarea
                            value={props.runnerEnvContent ?? ''}
                            onChange={(event) => props.onRunnerEnvChange?.(event.target.value)}
                            spellCheck={false}
                            className="min-h-56 w-full rounded-md border border-[var(--app-border)] bg-[var(--app-bg)] px-3 py-2 font-mono text-xs text-[var(--app-fg)]"
                        />
                        <div className="mt-2 flex flex-wrap gap-2">
                            <button
                                type="button"
                                onClick={props.onReloadRunnerEnv}
                                disabled={props.pendingAction === 'runner-env-load'}
                                className="rounded-md border border-[var(--app-border)] px-3 py-1.5 text-sm disabled:cursor-not-allowed disabled:opacity-60"
                            >
                                {props.pendingAction === 'runner-env-load' ? 'Loading…' : 'Reload env file'}
                            </button>
                            <button
                                type="button"
                                onClick={props.onSaveRunnerEnv}
                                disabled={props.pendingAction === 'runner-env-save'}
                                className="rounded-md border border-[var(--app-border)] px-3 py-1.5 text-sm disabled:cursor-not-allowed disabled:opacity-60"
                            >
                                {props.pendingAction === 'runner-env-save' ? 'Saving…' : 'Save env file'}
                            </button>
                        </div>
                        <div className="mt-2 text-xs text-[var(--app-hint)]">
                            Set these variables manually. Managed keys: ANTHROPIC_BASE_URL, ANTHROPIC_AUTH_TOKEN, ANTHROPIC_DEFAULT_OPUS_MODEL, ANTHROPIC_DEFAULT_SONNET_MODEL, ANTHROPIC_DEFAULT_HAIKU_MODEL, GOOGLE_GEMINI_BASE_URL, GEMINI_API_KEY. New agent sessions started by this runner will use them. Existing sessions keep their current environment.
                        </div>
                    </div>
                </div>
