...
function getInitialFontScale(): FontScale {
    return parseFontScale(safeGetItem(getFontScaleStorageKey()) ?? safeGetItem(getLegacyFontScaleStorageKey()))
}
...
export function useFontScale(): { fontScale: FontScale; setFontScale: (scale: FontScale) => void } {
    ...
    useEffect(() => {
        ...
        const onStorage = (event: StorageEvent) => {
            if (event.key !== getFontScaleStorageKey() && event.key !== getLegacyFontScaleStorageKey()) {
                return
            }
            const nextScale = parseFontScale(safeGetItem(getFontScaleStorageKey()) ?? event.newValue)
    ...
    const setFontScale = useCallback((scale: FontScale) => {
        ...
        if (scale === 1) {
            ...
            safeRemoveItem(getLegacyFontScaleStorageKey())
        } else {
            safeSetItem(getFontScaleStorageKey(), String(scale))
            safeSetItem(getLegacyFontScaleStorageKey(), String(scale))
        }
    }, [])
