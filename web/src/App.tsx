...
function AppInner() {
    ...
    const roomMatch = matchRoute({ to: '/rooms/$roomId' })
    ...
    const handleSseConnect = useCallback(() => {
        ...
        const invalidations = [
            ...
            ...(selectedRoomId ? [
                queryClient.invalidateQueries({ queryKey: queryKeys.room(selectedRoomId) }),
                queryClient.invalidateQueries({ queryKey: queryKeys.roomMessages(selectedRoomId) })
            ] : [])
        ]
    ...
    }, [api, queryClient, selectedRoomId, selectedSessionId, startSync, endSync])
    ...
    return (
        <AppContextProvider value={{ api, token, baseUrl, namespace: namespace ?? 'default' }}>
            <VoiceProvider>
